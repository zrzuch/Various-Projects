...
/******************************************************************
1) When a shape overlaps with another shape, the shape drawn first
hides the part of the shape that was drawn afterwards. It behaves
like this since it is simulating an object behind another object.
2) When a shape is drawn outside of the range [-1,1] for any coordinate,
it drawns part of the shape to simulate an object of the screen or 
out of view.
3) When you decrease the W coordinate, it seems to cause the center
of the shape to get closer or larger, but when you apply it to the
entire screen it looks like moving the camera in a game engine, so 
in our program, it causes the triangle to slowly take up the entire 
window. 
4) When the alpha attribute is lowered, it causes the shape or object
to blend with the object or shape or background behind it, so our
program looks like it fades to black.
******************************************************************/
...
initScene ()
{
  ...
  glVertexAttribPointer (COLOR_ATTRIB_INDEX, 4, GL_FLOAT, GL_FALSE, 0,
  ...
  ///////////////////////////////////////////////////////////////////
  // Use a third VAO for four vertex geometrty
  ...
  ///////////////////////////////////////////////////////////////////
  // Use a fourth VAO for triangle
  glBindVertexArray( g_vaos[3] );
  GLuint tri2Vbo;
  glGenBuffers (1, &tri2Vbo);
  glBindBuffer (GL_ARRAY_BUFFER, tri2Vbo);
  std::vector<float> tri2Vertices
    { // Vertices
      -0.9f, -0.9f, 
      -0.5f, -0.9f, 
      -0.5f, -0.5f, 
  ...
  glBufferData (GL_ARRAY_BUFFER, tri2Vertices.size () * sizeof(float),
    tri2Vertices.data (),
    GL_STATIC_DRAW);
  glEnableVertexAttribArray (POSITION_ATTRIB_INDEX);
  glVertexAttribPointer (POSITION_ATTRIB_INDEX, 2, GL_FLOAT, GL_FALSE, 0,
