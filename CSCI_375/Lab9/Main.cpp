...
initScene ()
{
    ...
    g_scene->lights[0].setIsDirect(false);
    g_scene->lights[0].setAngleOfLight(0.5f);
    g_scene->lights[0].direction = {0, 0, -1};
    g_scene->lights[0].intensity.setSpecular({1});
    g_scene->lights[0].intensity.setDiffuse ({1});
    ...
    g_scene->models->add(g_meshFiles[0], begin);
    g_scene->models->add(g_meshFiles[1]);
    g_scene->models->add(g_meshFiles[2]);

    g_scene->models->getModel("Sphere.obj")->getTransform(0).moveUp(10);
    g_scene->models->getModel("Sphere.obj")->getTransform(0).moveBack(10);
    g_scene->models->getModel("Sphere.obj")->getTransform(0).moveRight(5);
...
updateScene (double deltaTime)
{
    ...
    std::string str = std::to_string(1 / deltaTime) + " fps";
