...
{
	return getViewMatrix(true).getOrientation(true);
}
...
// You can use this matrix as the model-view matrix
const Transform&
Camera::getViewMatrix (bool getConst) const
{
	if ( m_updateView )
	{
		m_modelView = m_world;
...
void
Camera::zoom (double verticalFovDelta)
{
	if (m_currProjection == ProjectionType::SYMMETRIC)
	{
		m_fov = Math::clamp(m_fov - verticalFovDelta, 1.0, 120.0);
		m_updateProjection = true;
	}
}

void
Camera::setAspectRatio(double aspectRatio)
{
...
void
Camera::setProjection(double verticalFovDegrees)
{
...
void
Camera::setProjection (double verticalFovDegrees, double aspectRatio,
...
Camera::getXFOV() const
{
	if (m_currProjection == ProjectionType::SYMMETRIC)
	{
		return m_aspectRatio * m_fov;
	}
	else
	{
		return Math::toDegrees(atan(m_right / m_nearZ)) * 2;
	}
}
...
Camera::getYFOV() const
{
	...
	{
		return Math::toDegrees(atan(m_top / m_nearZ)) * 2;
	}
}