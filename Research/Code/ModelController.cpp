...
{
	// Dragon Sphere for Debuging
	// Transform begin;
	// begin.scaleLocal(getModel("Deathwing.obj")->getRadius());
	// begin.setPosition(getModel("Deathwing.obj")->getCenter());
	// add("Sphere.obj");
	// getModel("Sphere.obj")->getTransform(0) = begin;
	for (uint i = 0; i < numModel(); ++i)
	{
		getModel(i)->prepare();
	}
...
ModelController::draw(ShaderProgram* shaderProgram, const Camera& camera, bool isShaderOn)
{
	...
	for (std::pair<Model*, bool> modelPair : m_models)
	{
		if (modelPair.second)
		{
			numTriangles += modelPair.first->draw(shaderProgram, camera, true);
		}
	}
