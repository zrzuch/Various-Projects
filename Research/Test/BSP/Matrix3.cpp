...
Matrix3::Matrix3(float rx, float uy, float bz)
	: m_right( rx, 0, 0 ), m_up( 0, uy, 0 ), m_back( 0, 0, bz )
{ }
...
void
Matrix3::scaleMatrix(const Vector3& scale)
{
	m_right *= scale.x;
	m_up 	*= scale.y;
	m_back 	*= scale.z;
...
float&
Matrix3::getValue(unsigned i, unsigned j)
{
	return *(data() + (j * 3) + i);
}
